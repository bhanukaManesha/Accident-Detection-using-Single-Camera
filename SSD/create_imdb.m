function [net, stats] = create_imdb(varargin)
% CNN_TOY_DATA
...
opts.train.expDir = [pwd '/kitti_dataset'] ;
opts.dataDir = [pwd '/kitti_dataset'] ;
...
sets = {'train', 'test'} ;
numSamples = [6001, 1480] ;

% Preallocate memory
totalSamples = 7481 ;  
images = zeros(375, 1242, 3, totalSamples, 'single') ;
...
for s = 1:2  % Iterate sets
    for i = 1:numSamples(s)  % Iterate samples
      % Read image
      im = imread(sprintf('%s/%s/%06i.png', dataDir, sets{s}, i)) ;
